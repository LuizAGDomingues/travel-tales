...
function ExploreButtonsIconStart({ icon, text, onClick }: ExploreButtonProps) {
  return (
    <Button
      ...
      onClick={onClick}
      startIcon={icon}
      style={{textTransform: 'none', border: 'none', marginTop: '2.25rem', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.3)', borderRadius: '10px', cursor: 'none'}}
      className='py-4 px-6 text-black cursor-none'
    >
      <Text text={text}/>
    </Button>
  )
