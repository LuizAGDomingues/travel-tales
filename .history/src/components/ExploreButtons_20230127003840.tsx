...
function ExploreButtonsIconStart({ icon, text, onClick }: ExploreButtonProps) {
  return (
    <Button
      ...
      onClick={onClick}
      startIcon={icon}
      style={{textTransform: 'none', border: 'none', marginTop: '2.25rem', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.3)', borderRadius: '10px', cursor: 'auto'}}
...
function ExploreButtonsIconBoth({ iconStart, iconEnd, text, className, link = '', onClick }: ExploreButtonsIconBoth) {
  return (
    <Button 
      onClick={onClick}
      variant="outlined"
      ...
      className={`py-4 px-6 text-black w-[31rem] ${class}`}
    >
      <Text text={text}/>
    </Button>
  )
