...
export async function getNearbyPlaces(latitude: string, longitude: string, type: string) {
  ...
  const url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${location}&radius=50000&type=${type}&key=${GOOGLE_MAPS_API_KEY}`;
  try {
    const response = await axios.get(url);
    const places = response.data.results;
    getClosestPlace(parseFloat(latitude), parseFloat(longitude), )
  } catch (error) {
    console.error(error);
    return error;
  }
