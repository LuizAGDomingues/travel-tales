...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { latitude, longitude, type } = req.headers;
  try {
    ...
    const nearestPlace = await getNearbyPlaces(latitude.toString(), longitude.toString(), type.toString())
    return res.send(nearestPlace)
  } catch (err: any) {
    return res.json({
      error: true,
      message: err.message
...
async function getNearbyPlaces(latitude: string, longitude: string, type: string) {
  ...
  const url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${location}&radius=5000&keyword=${}&type=${type}&key=${GOOGLE_MAPS_API_KEY}`;
  try {
