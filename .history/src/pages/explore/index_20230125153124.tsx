...
export default function Home() {
  ...
  useEffect(() => {
    async function nearbyLocations() {
      ...
      const nearbyMuseum = await getNearbyPlaces(userLatitude, userLongitude, 'museum')
      const nearbyShopping = await getNearbyPlaces(userLatitude, userLongitude, 'shopping')
      const nearbyRestaurant = await getNearbyPlaces(userLatitude, userLongitude, 'restaurant')
      ...
      <main>
        ...
        <section className='flex flex-col items-center justify-center w-full mt-[20rem]'>
          ...
          <div className='flex flex-wrap mb-10 items-center justify-center gap-5'>
            ...
            <ExploreButtons.IconEnd onClick={handleClickPark} icon={<MuseumIcon sx={{ color: '#000', width: '32px', height: '32px' }}/>} text={closestMuseum.name} />
            <ExploreButtons.IconEnd onClick={handleClickPark} icon={<ShoppingBagIcon sx={{ color: '#000', width: '32px', height: '32px' }}/>} text={closestShopping.name} />
            <ExploreButtons.IconEnd onClick={handleClickPark} icon={<LocalDiningIcon sx={{ color: '#000', width: '32px', height: '32px' }}/>} text={closestRestaurant.name} />
          </div>
        </section>
      </main>
    </>
