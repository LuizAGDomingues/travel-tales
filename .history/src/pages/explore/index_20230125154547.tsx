  ...
  useEffect(() => {
    async function nearbyLocations() {
      const [nearbyPark, nearbyMuseum, nearbyShopping, nearbyRestaurant] = await Promise.all([
        ...
        getNearbyPlaces(userLatitude, userLongitude, 'shop'),
        getNearbyPlaces(userLatitude, userLongitude, 'restaurant')
      ])

      setClosestPark({
        name: nearbyPark && nearbyPark.name,
        lat: nearbyPark && nearbyPark.geometry.location.lat,
        lng: nearbyPark && nearbyPark.geometry.location.lng
      })
      setClosestMuseum({
        name: nearbyMuseum && nearbyMuseum.name,
        lat: nearbyMuseum && nearbyMuseum.geometry.location.lat,
        lng: nearbyMuseum && nearbyMuseum.geometry.location.lng
      })
      setClosestShopping({
        name: nearbyShopping && nearbyShopping.name,
        lat: nearbyShopping && nearbyShopping.geometry.location.lat,
        lng: nearbyShopping && nearbyShopping.geometry.location.lng
      })
      setClosestRestaurant({
        name: nearbyRestaurant && nearbyRestaurant.name,
        lat: nearbyRestaurant && nearbyRestaurant.geometry.location.lat,
        lng: nearbyRestaurant && nearbyRestaurant.geometry.location.lng
      })
    }
    nearbyLocations()
