...
export default function Home() {
  ...
  const [closestMuseum, setClosestMuseum] = useState({
    name: '',
    lat: '',
    lng: ''
  });
  const [closestShopping, setClosestShopping] = useState({
    name: '',
    lat: '',
    lng: ''
  });
  ...
  useEffect(() => {
    async function nearbyLocations() {
      const [nearbyPark, nearbyMuseum, nearbyShopping, nearbyRestaurant]: any =
        await Promise.all([
      ...
      if (
        nearbyPark.name != undefined &&
        nearbyMuseum.name != undefined &&
        nearbyShopping.name != undefined &&
        nearbyRestaurant.name != undefined
      ) {
        setClosestPark({
        ...
        <section className="flex flex-col items-center justify-center w-full mt-[20rem]">
          ...
          <div className="flex items-center justify-center gap-7">
            <ExploreButtons.IconStart
              icon={
                <TripOriginOutlinedIcon
                  sx={{ color: '#4285F4', width: '32px', height: '32px' }}
                />
              }
              text="Your location"
            />
            <ExploreButtons.IconStart
              icon={
                <LocationOnIcon
                  sx={{ color: '#DB4437', width: '32px', height: '32px' }}
                />
              }
              text="Nearest spot"
            />
            ...
            <ExploreButtons.IconEnd
              ...
              icon={
                isLoading ? (
                  ...
                  <ParkIcon
                    sx={{ color: '#0F9D58', width: '32px', height: '32px' }}
                  />
                )
              }
